...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # mlflow.set_tag('dataset_bucket', opt.dvc_bucket)  # ! to check
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if signature is None:
                    # out_pred, image = run_inference(model, device, opt.imgsz)
                    signature = infer_signature(
                        imgs.cpu().numpy(), pred[0][0].detach().cpu().numpy()
                    )
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if best_fitness == fi:
                    ...
                    mlflow.pytorch.log_model(
                        ckpt["model"], "yolov7", signature=signature
                    )
    ...
    # print(f'artifact path {opt.dvc_bucket} (just print)')
    shutil.rmtree(Path(opt.save_dir) / "weights/") # !!! here
    # mlflow.log_artifacts(opt.save_dir, artifact_path=opt.dvc_bucket)
    mlflow.log_artifacts(opt.save_dir, artifact_path=opt.artifact_name)
...
def main(opt, callbacks=Callbacks()):
    mlflow.set_experiment(os.getenv('MLFLOW_EXP_NAME'))
    mlflow.start_run()
    # Checks
    if RANK in {-1, 0}:
        print_args(vars(opt))
